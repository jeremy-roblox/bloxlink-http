...
async def parse_nickname(
    ...
    template: str,
    roblox_user=MISSING,
    group: groups.RobloxGroup = MISSING,
    ...
    if roblox_user is not MISSING:
        ...
        template = (
            template.replace("roblox-name", roblox_user.username)
            .replace(
                "display-name",
            ...
            .replace("roblox-id", str(roblox_user.id))
            .replace(
                "roblox-age",
                str(roblox_user.age)
                # ).replace(
        ...
        if not template:
            template: str | None = (
                await bloxlink.fetch_guild_data(guild, "unverifiedNickname")
            ).unverifiedNickname
    ...
    template = (
        template.replace("discord-name", member.user.username)
        .replace("discord-nick", member.display_name)
        .replace("discord-mention", member.user.mention)
        .replace("discord-id", str(member.id))
        .replace("server-name", guild.name)
        .replace("prefix", "/")
        .replace("group-url", group.url if group else "")
        .replace("group-name", group.name if group else "")
    )
