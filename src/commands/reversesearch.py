...
from resources.exceptions import BloxlinkException, RobloxNotFound  # pylint: disable=import-error, no-name-in-module
    ...
    async def __main__(self, ctx: CommandContext):
        ...
        try:
            if search_type == "username":
                json_data = await fetch("POST", "https://users.roblox.com/v1/usernames/users", body={"usernames": [target], "excludeBannedUsers": False})
            ...
            num_documents = await bloxlink.mongo.bloxlink.users.count_documents({ "robloxAccounts.accounts": roblox_id})
