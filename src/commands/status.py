...
class StatusCommand:
    ...
    async def __main__(self, ctx: CommandContext):
        ...
        if premium_status.active:
            ...
            embed.add_field(
                name="Tier",
                value=premium_status.tier,
                inline=True
            )
            embed.add_field(
                name="Payment Source",
                value=premium_status.charge_source,
                inline=True
            )
            embed.add_field(
                name="Unlocked Features",
                value=str(premium_status),
                inline=True
            )
        else:
            embed.description = "This server does not have premium. A server admin may purchase it [here](https://blox.link)."
